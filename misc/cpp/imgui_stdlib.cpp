...
bool ImGui::InputTextMultiline_WithGeometry(const char* label, std::string* str, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImMulitlineTextGeometryData* geometry)
{
    IM_ASSERT((flags & ImGuiInputTextFlags_CallbackResize) == 0);
    flags |= ImGuiInputTextFlags_CallbackResize;
    ...
    cb_user_data.ChainCallback = callback;
    cb_user_data.ChainCallbackUserData = user_data;
    return ImGui::InputTextMultilineWithGeometry(label, (char*)str->c_str(), str->capacity() + 1, size, flags, InputTextCallback, &cb_user_data, geometry);
}
